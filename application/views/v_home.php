<!DOCTYPE html>
<html>

  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta name="description" content="">
    <meta name="author" content="">

    <title>LOMBA KITA</title>

    <!-- Bootstrap core CSS -->
    <link href="<?php echo base_url();?>assets/css/bootstrap.min.css" rel="stylesheet">

    <!-- Custom styles -->
    <link href="<?php echo base_url();?>assets/css/modern-business.css" rel="stylesheet">


 



  </head>

  <body>


    <?php
      include_once("navbar.php");
        

        
    ?>
	</br></br></br>	
    <!-- <div>test</div> -->
    <div class="container-fluid">

    <div id="carouselExampleIndicators" class="carousel slide" data-ride="carousel">
  		<ol class="carousel-indicators">
		    <li data-target="#carouselExampleIndicators" data-slide-to="0" class="active"></li>
		    <li data-target="#carouselExampleIndicators" data-slide-to="1"></li>
		    <li data-target="#carouselExampleIndicators" data-slide-to="2"></li>
  		</ol>
  <div class="carousel-inner">
    <div class="carousel-item active">
      <img class="d-block w-100" src="<?php echo base_url(); ?>assets/img/first.jpg" alt="First slide">
      <div class="carousel-caption d-none d-sm-block">
        <h3>Posting Perlombaan</h3>
        <p><div class="text_to_html"><p>Organizer dapat mendaftarkan perlombaan di situs ini dan sekaligus melakukan manajemen pada platform lombakita.</p></div></p>
      </div>
    </div>
    <div class="carousel-item">
      <img class="d-block w-100" src="<?php echo base_url(); ?>assets/img/second.jpg" alt="Second slide">
      <div class="carousel-caption d-none d-sm-block">
        <h3>Ikuti Perlombaan</h3>
        <p><div class="text_to_html"><p>Mengikuti berbagai macam perlombaan</p></div></p>
      </div>
    </div>
    <div class="carousel-item">
      <img class="d-block w-100" src="<?php echo base_url(); ?>assets/img/third.png" alt="Third slide">
      <div class="carousel-caption d-none d-sm-block">
        <h3>Manajemen Perlombaan</h3>
        <p><div class="text_to_html"><p>Permudah manajemen perlombaan anda</p></div></p>
      </div>
    </div>
  </div>
  <a class="carousel-control-prev" href="#carouselExampleIndicators" role="button" data-slide="prev">
    <span class="carousel-control-prev-icon" aria-hidden="true"></span>
    <span class="sr-only">Previous</span>
  </a>
  <a class="carousel-control-next" href="#carouselExampleIndicators" role="button" data-slide="next">
    <span class="carousel-control-next-icon" aria-hidden="true"></span>
    <span class="sr-only">Next</span>
  </a>
</div>
</br></br>
<div id="page-content" class="row">
            <div id="region-main-box" class="col-12">
                <section id="region-main" >
                    <div class="card">
                        <div class="card-body">
                            <div class="fpwonderbox">
    <div class="container-fluid">
    <div class="row">
    <div class="col-md-12">



</div>
</div>
</div>

<div class="container-fluid">
    <div class="row">
    <div class="col-md-12">

                <div class="fptextbox">
                    
                </div>

    </div>
    </div>
    </div>


<div class="markettiles">
    <div class="container-fluid">
    <div class="row">


        <div class="col-md-4">
        <div class="market-tile" style="background-color: #F5F5F5; padding: 75px; margin-left: 0px; margin-right: -15px" > 
            <h3 class="card-title" style="margin-left: -60px;">Create</h3>
            <div class="marketcontent" >
            <p><div class="text_to_html" style="background-color: white; padding: 8px; margin-left: -60px; margin-right: -60px"><p>Membuat berbagai jenis perlombaan berbasis online</p></div><p>
            </div>
            <a href = '#' class='btn btn-primary' style="margin-left: -60px; margin-bottom: -120px; width:420px">  </a> 
        </div>
        </div>


        <div class="col-md-4">
        <div class="market-tile" style="background-color: #F5F5F5; padding: 75px; margin-left: 0px; margin-right: -15px"> 
            <h3 class="card-title" style="margin-left: -60px;">Share</h3>
            <div class="marketcontent ">
            <p><div class="text_to_html" style="background-color: white; padding: 8px; margin-left: -60px; margin-right: -60px"><p>Bagikan eventmu di platform manajemen perlombaan</p></div><p>
            </div>
        	<a href = '#' class='btn btn-primary ' style="margin-left: -60px; margin-bottom: -120px; width:420px"></a>
        </div>
        </div>


        <div class="col-md-4">
        <div class="market-tile" style="background-color: #F5F5F5; padding: 75px; margin-left: 0px; margin-right: -15px"> 
            <h3 class="card-title" style="margin-left: -60px;">Follow</h3>
            <div class="marketcontent ">
            <p><div class="text_to_html" style="background-color: white; padding: 8px; margin-left: -60px; margin-right: -60px"><p>Ikuti berbagai perlombaan di LombaKita</p></div><p>
            </div>
            <a href = '#' class='btn btn-primary' style="margin-left: -60px; margin-bottom: -120px; width:420px">  </a> 
        </div>
        </div>


    </div>
    </div>
</div>
</br>

<div><h2 style="margin-left: 15px">Competition categories</h2></br></br>
        <div class="content" style="margin-left: 170px"><div class="subcategories d-flex flex-wrap">
            <div class="category loaded col-md-3 box-class" data-categoryid="2" data-depth="1" data-showcourses="5" data-type="0">
                <div class="cat-icon">
                    <a href="#"><img src="<?php echo base_url(); ?>assets/img/circle.png" width="66px" height="66px"></br>
                        <div>
                         	<div class="info-enhanced"><span class="class-category">Pelajar</span>  <span class="numberofcourses" title="Number of Competitions">(3)</span></div></div></a></div></div>

            <div class="category loaded col-md-3 box-class" data-categoryid="3" data-depth="1" data-showcourses="5" data-type="0">
            	<div class="cat-icon">
            		<a href="#"> <img src="<?php echo base_url(); ?>assets/img/circle.png" width="66px" height="66px"></br>
                        <div>
                        	<div class="info-enhanced"><span class="class-category" style="margin-left: -20px">Mahasiswa</span>  
                            		<span class="numberofcourses" title="Number of Competitions">(10)</span></div></div></a></div></div>
            <div class="category loaded col-md-3 box-class" data-categoryid="4" data-depth="1" data-showcourses="5" data-type="0">
                <div class="cat-icon">
                	<a href="#"><img src="<?php echo base_url(); ?>assets/img/circle.png" width="66px" height="66px"></br>
                		<div>
                			<div class="info-enhanced"><span class="class-category">Umum</span>  <span class="numberofcourses" title="Number of Competitions">(6)</span></div></div></a></div></div>
            <div class="category loaded col-md-3 box-class" data-categoryid="1" data-depth="1" data-showcourses="5" data-type="0" >
                <div class="cat-icon"><a href="#"><img src="<?php echo base_url(); ?>assets/img/circle.png" width="66px" height="66px"></br>
                	<div>
                		<div class="info-enhanced"><span class="class-category">Lainnya</span>  <span class="numberofcourses" title="Number of Competitions">(1)</span></div></div></a></div></div>
        </div></div>


        <div id="category-course-list">
                    <div class="courses category-course-list-all">
                    <hr>
                    <div class="class-list">
                        <h4>Available competitions</h4>
                    </div><div class="container-fluid"> <div class="row">
                        <div class="col-md-4"><div class="coursevisible">
                            <div class="class-box">
                                
                                    <a data-tooltip="tooltip" data-placement= "top" title="Electronics And Informatics Competition" href="#">
                                    <div class="courseimagecontainer">
                                    <div class="course-image-view"><img src="<?php echo base_url(); ?>assets/img/cover.jpg" height="300px">
                                    </div>
                                    
                                    
                                    </div>
                                    <div class="course-title">
                                    <h4>Electronics And Informatics Competition</h4>
                                    </div>
                                    </a>
                                    <div class="course-summary">
                                    <div class="coursecat">Category: <a class="" href="#">Pelajar</a></div>
                                    
                                    </div>
                                </div>
                        </div>
                        </div>
                        <div class="col-md-4"><div class="coursevisible">
                            <div class="class-box">
                                
                                    <a data-tooltip="tooltip" data-placement= "top" title="iStudy: STAN Try Out Online 2018" href="#">
                                    <div class="courseimagecontainer">
                                    <div class="course-image-view"> <img src="<?php echo base_url(); ?>assets/img/cover.jpg" height="300px">
                                    </div>
                                    
                                    
                                    </div>
                                    <div class="course-title">
                                    <h4>iStudy: STAN Try Out Online 2018</h4>
                                    </div>
                                    </a>
                                    <div class="course-summary">
                                    <div class="coursecat">Category: <a class="" href="#">Pelajar</a></div>
                                    
                                    </div>
                                </div>
                        </div>
                        </div>
                        <div class="col-md-4"><div class="coursevisible">
                            <div class="class-box">
                                
                                    <a data-tooltip="tooltip" data-placement= "top" title="ELECTRICAL COMPETITION 2017" href="#">
                                    <div class="courseimagecontainer">
                                    <div class="course-image-view"> <img src="<?php echo base_url(); ?>assets/img/cover3.jpg" height="300px" width="400px">
                                    </div>
                                    
                                    
                                    </div>
                                    <div class="course-title">
                                    <h4>ELECTRICAL COMPETITION 2017</h4>
                                    </div>
                                    </a>
                                    <div class="course-summary">
                                    <div class="coursecat">Category: <a class="" href="#">Pelajar</a></div>
                                    
                                    </div>
                                </div>
                        </div>
                        </div></div> </div>
                        <div class="container-fluid"> <div class="row">
                        <div class="col-md-4"><div class="coursevisible">
                            <div class="class-box">
                                
                                    <a data-tooltip="tooltip" data-placement= "top" title="UNY- NATIONAL INNOVATION AND CREATION OF TECHNOLOGY" href="#">
                                    <div class="courseimagecontainer">
                                    <div class="course-image-view"> <img src="<?php echo base_url(); ?>assets/img/cover.jpg" height="300px">
                                    </div>
                                    
                                    
                                    </div>
                                    <div class="course-title">
                                    <h4>UNY- NATIONAL INNOVATION AND CREATION OF TECHNOLOGY</h4>
                                    </div>
                                    </a>
                                    <div class="course-summary">
                                    <div class="coursecat">Category: <a class="" href="#">Mahasiswa</a></div>
                                    
                                    </div>
                                </div>
                        </div>
                        </div>
                        <div class="col-md-4"><div class="coursevisible">
                            <div class="class-box">
                                
                                    <a data-tooltip="tooltip" data-placement= "top" title="UNITY#7 2019 - Network Computer Competition" href="#">
                                    <div class="courseimagecontainer">
                                    <div class="course-image-view"> <img src="<?php echo base_url(); ?>assets/img/cover.jpg" height="300px">
                                    </div>
                                    
                                    
                                    </div>
                                    <div class="course-title">
                                    <h4>UNITY#7 2019 - Network Computer Competition</h4>
                                    </div>
                                    </a>
                                    <div class="course-summary">
                                    <div class="coursecat">Category: <a class="" href="#">Mahasiswa</a></div>
                                    
                                    </div>
                                </div>
                        </div>
                        </div>
                        <div class="col-md-4"><div class="coursevisible">
                            <div class="class-box">
                                
                                    <a data-tooltip="tooltip" data-placement= "top" title="UNITY#7 2019 - Software Development Competition" href="#">
                                    <div class="courseimagecontainer">
                                    <div class="course-image-view"> <img src="<?php echo base_url(); ?>assets/img/cover.jpg" height="300px">
                                    </div>
                                    
                                    
                                    </div>
                                    <div class="course-title">
                                    <h4>UNITY#7 2019 - Software Development Competition</h4>
                                    </div>
                                    </a>
                                    <div class="course-summary">
                                    <div class="coursecat">Category: <a class="" href="#">Mahasiswa</a></div>
                                    
                                    </div>
                                </div>
                        </div>
                        </div></div> </div><div class="container-fluid"> <div class="row">
                        <div class="col-md-4"><div class="coursevisible">
                            <div class="class-box">
                                
                                    <a data-tooltip="tooltip" data-placement= "top" title="INFINITE Selection 2019" href="#">
                                    <div class="courseimagecontainer">
                                    <div class="course-image-view"> <img src="<?php echo base_url(); ?>assets/img/cover.jpg" height="300px">
                                    </div>
                                    
                                    
                                    </div>
                                    <div class="course-title">
                                    <h4>INFINITE Selection 2019</h4>
                                    </div>
                                    </a>
                                    <div class="course-summary">
                                    <div class="coursecat">Category: <a class="" href="#">Mahasiswa</a></div>
                                    
                                    </div>
                                </div>
                        </div>
                        </div>
                        <div class="col-md-4"><div class="coursevisible">
                            <div class="class-box">
                                
                                    <a data-tooltip="tooltip" data-placement= "top" title="LombaKita Mini Competion 2018" href="#">
                                    <div class="courseimagecontainer">
                                    <div class="course-image-view"> <img src="<?php echo base_url(); ?>assets/img/cover.jpg" height="300px">
                                    </div>
                                    
                                    
                                    </div>
                                    <div class="course-title">
                                    <h4>LombaKita Mini Competion 2018</h4>
                                    </div>
                                    </a>
                                    <div class="course-summary">
                                    <div class="coursecat">Category: <a class="" href="#">Mahasiswa</a></div>
                                    
                                    </div>
                                </div>
                        </div>
                        </div>
                        <div class="col-md-4"><div class="coursevisible">
                            <div class="class-box">
                                
                                    <a data-tooltip="tooltip" data-placement= "top" title="CTF INFINITE UNY" href="#">
                                    <div class="courseimagecontainer">
                                    <div class="course-image-view"> <img src="<?php echo base_url(); ?>assets/img/cover2.jpg" height="300px" width="400px">
                                    </div>
                                    
                                    
                                    </div>
                                    <div class="course-title">
                                    <h4>CTF INFINITE UNY</h4>
                                    </div>
                                    </a>
                                    <div class="course-summary">
                                    <div class="coursecat">Category: <a class="" href="#">Mahasiswa</a></div>
                                    
                                    </div>
                                </div>
                        </div>
                        </div></div> </div>
                    </div>
                </div><br /></div>
                            
                            
                        </div>
                    </div>
                </section>


            </div>
        </div>
    </div>
</div>

        

<footer style="background-color: #2F4F4F">
    </br>
    <div class="container">

        <div class="row">

                <div class="col-md-8">
                    <div class="text_to_html">
                    	<div class="row">
                    			<div class="column">
                    					<h4 style="color: white">Informasi Perusahaan   </h4>
                    					<u><p ><a href="#" style="color: white">Tentang Kami</a></p>
                    					<p><a href="#" style="color: white">Karir</a></p>
                    					<p><a href="#" style="color: white">Kontak</a></p>
                    					<p><a href="#" style="color: white">Blog</a></p></u>
                				</div>
                				<div class="column">
                						<h4 style="color: white">Pelajari Lebih      </h4>
                						<u><p><a href="#" style="color: white">Fitur</a></p>
                						<p><a href="#" style="color: white">Kisah Sukses</a></p>
                						<p><a href="#" style="color: white">Cara Transaksi</a></p>
                						<p><a href="#" style="color: white">Penyelenggara Lomba</a></p></u>
                				</div>
                				<div class="column">
                						<h4 style="color: white">Layanan Lainnya</h4>
                						<u><p><a href="#" style="color: white">Toko Perlombaan</a></p></u>
                				</div>
                		</div>
                	</div>

                </div>

                <div class="col-md-4">
                    

                        <h3 style="color: white">LombaKita Indonesia</h3>
                            <ul style="color: white">
                                <li>  https://lombakita.com</li>
                                <li>  info@lombakita.com</li>
                                <li>  +62815 4274 4654</li>
                            </ul>

                </div>
        </div>

    </div>
</footer>
                            



  </body>

</html>
